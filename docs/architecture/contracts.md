...
The `dataset.deep.requested` consumer validates the section request before dispatching to a registered processor. Processor registration is keyed by section token and is deployment configuration; the event envelope remains provider-neutral. The reference enrichment image registers `search` and `quality`: `search` reads only approved evidence for the requested projection's source snapshot and publishes an evidence-backed source index, while `quality` publishes a deterministic confidence/artifact report over the same approved evidence. Neither processor infers canonical facts. A missing processor is a terminal configuration error and is dead-lettered, so domain sections remain unavailable until their dedicated extractor is deployed. A registered processor receives the validated request identity, reads source material through its secret-managed runtime interfaces, and must publish through the deep immutable-revision contract or return a retryable failure. No processor may mutate an earlier published revision.
