...
The ingestion worker also contains a durable pull-consumer boundary for version-one `dataset.fast.requested` events. It is disabled by default until the transactional fast-lane projection publisher is enabled. When that handler is deployed, set `AUTODATA_FAST_CONSUMER_ENABLED=1`; use `AUTODATA_FAST_CONSUMER_DURABLE` for the stable consumer name and `AUTODATA_FAST_CONSUMER_MAX_DELIVERIES` for the bounded delivery limit. The consumer acknowledges only successful handler completion, applies exponential `nak` delays to retryable failures, immediately dead-letters malformed envelopes, and publishes exhausted work to `dataset.fast.dead_letter` with a stable NATS message ID. It never receives source credentials from an event.
