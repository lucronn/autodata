...
- [ ] Create or identify the dedicated GitHub Issue for the README, visual, and Wiki refresh, then add or identify its item in Project #8.
- [ ] Add the exact Issue URL, Project URL, concrete work items, and synchronized status to this plan, `docs/public-facing/README-and-wiki.md`, and `docs/github/operating-model.md`.
- [ ] Commit the plan and canonical-document checkpoint, verify the staged path list excludes `sample data/`, and record the checkpoint SHA.
- [ ] Run the repository preflight against that exact checkpoint SHA; stop if the record is missing, stale, malformed, or unsynchronized.
...
- [ ] Generate a polished wide visual for the repository front page: a clean, dark-on-light automotive data flow with a vehicle silhouette or repair-data motif, connected source/evidence/data/lane shapes, no tiny copy, no invented product claims, no watermark, and no embedded text that must be read as documentation.
- [ ] Inspect the generated image, copy it to `docs/assets/autodata-platform-overview.png`, and confirm its dimensions and format.
- [ ] Remove the remote `repository-images.githubusercontent.com` embed and the obsolete JPEG reference from `README.md`; place the replacement image once near the title.
- [ ] Add a short navigation row for the Wiki, architecture docs, local setup, current slice, and Project #8; preserve the existing detailed commands below it while tightening inaccurate or duplicate opening prose.
- [ ] Search the repository for references to the deleted JPEG and fail the task if any remain.
...
- [ ] Write `Home.md` with the product summary, current status boundary, navigation links, and a clear statement that repository `docs/` remains authoritative.
- [ ] Write `Architecture.md` as a concise map to the domain model, fast/deep lifecycle, contracts, infrastructure, and the single replacement visual.
- [ ] Write `Getting-Started.md` with the deterministic Compose path, dashboard URL, smoke-test intent, and links back to the full development guide.
- [ ] Write `Contributing.md` with the operating model, Issue/Project workflow, documentation source rule, pre-implementation gate, and secret-handling boundary.
- [ ] Clone or initialize the GitHub Wiki repository only after confirming the target repository and authenticated identity; publish only the four requested pages.
- [ ] Verify that each published page is readable from the Wiki remote and that every repository-relative link resolves from the corresponding Wiki context or uses an absolute repository URL.
...
- [ ] Run `git diff --check`, Markdown fence/link/reference scans, secret-marker scans, the repository-governance tests, and the applicable documentation CI checks.
- [ ] Inspect the rendered README and replacement image, confirming the title, navigation, image count, local setup commands, and review-status disclaimer are all visible and accurate.
- [ ] Re-read the GitHub Issue and Project #8 item, update their status and exact commit/source references, and preserve the repository-docs-as-authority statement.
- [ ] Commit only the requested README, asset, canonical docs, plan, and Wiki source paths; inspect `git diff --cached --name-only` before committing.
- [ ] Push the requested repository branch, verify the local SHA matches the remote SHA, and read back the Wiki remote’s page commit and the Issue/Project records.

## Expected outcome

The GitHub repository has a clear public front page with one intentional visual, the obsolete rough visual and redundant remote embed are gone, and the Wiki provides a useful first-time path through AutoData while linking back to canonical repository documentation for all technical truth.
