(() => {
  ...
  function renderConsumerSummary(payload, answer) {
    ...
    if (procedure || quote) {
      ...
      message = "Choose the matching vehicle below to load the exact vehicle-matched procedure and quote.";
    } else if (text(payload.status) === "failed" || dataStateFrom(payload, answer) === "unavailable") {
      message = "I couldn’t complete the lookup yet. Any available vehicle-matched information is shown below.";
    }
  ...
  function friendlyWarning(warning) {
    ...
    if (normalized.includes("procedure_content_unavailable")) return "The source returned a labor label but not the repair instructions yet. This is not a complete procedure.";
  ...
  function renderProcedure(payload, answer) {
    ...
    setText("procedure-generation", procedure.content_status === "complete"
    ...
    if (!steps.length) {
      ...
      replaceChildren($("procedure-steps"), steps.map((step) => {
        ...
        const action = pick(step, ["action", "description", "text"]);
        item.append(element("span", "step-instruction", action || JSON.stringify(step)));
        const instructions = values(step.instructions);
        if (instructions.length) {
          const detail = element("div", "step-detail");
          instructions.forEach((instruction) => detail.append(element("p", "step-detail-line", instruction)));
          item.append(detail);
        }
        const stepImages = values(step.images).filter((image) => /^https?:\/\//i.test(visualURL(image)));
        if (stepImages.length) {
          const figures = element("div", "step-visuals");
          stepImages.forEach((visual) => {
            const figure = element("figure", "step-visual");
            const image = document.createElement("img");
            image.src = visualURL(visual);
            image.alt = text(pick(visual, ["alt", "description"])) || "Procedure figure";
            image.loading = "lazy";
            figure.append(image);
  ...
  function renderVisuals(payload, answer, procedure) {
    ...
    if (values(procedure.steps).some((step) => values(step.images).length)) {
      target.hidden = true;
      replaceChildren(target, []);
      return;
    }
