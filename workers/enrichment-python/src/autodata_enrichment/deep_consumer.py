"""Durable JetStream delivery for deep-section execution requests."""
...
from .deep_dispatch import DeepRequest, DeepRequestError, parse_deep_request


DEEP_SUBJECT = "dataset.deep.requested"
DEEP_DEAD_LETTER_SUBJECT = "dataset.deep.dead_letter"
DEFAULT_STREAM = "AUTODATA"
DEFAULT_DURABLE = "autodata-deep-sections"


async def consume_once(
    handler: Callable[[DeepRequest], Any],
    *,
    connect: Callable[[str], Awaitable[Any]] | None = None,
    url: str | None = None,
    stream: str | None = None,
    durable: str | None = None,
    subject: str = DEEP_SUBJECT,
    dead_letter_subject: str = DEEP_DEAD_LETTER_SUBJECT,
    fetch_timeout: float = 1,
    max_deliveries: int = 3,
) -> dict[str, Any]:
    """Process at most one deep request with at-least-once delivery."""
    ...
    durable = durable or os.getenv("AUTODATA_DEEP_CONSUMER_DURABLE", DEFAULT_DURABLE)
    ...
    try:
        ...
        try:
            ...
            if isinstance(error, DeepRequestError) or delivery_count >= max_deliveries:
                await _dead_letter(
                    jetstream,
                    dead_letter_subject,
...
def _decode_envelope(data: bytes) -> dict[str, Any]:
    try:
        ...
        raise DeepRequestError("deep message is not valid JSON") from error
    if not isinstance(envelope, dict):
        raise DeepRequestError("deep message envelope must be an object")
    return envelope
...
async def _dead_letter(
    ...
    await jetstream.publish(
        ...
        headers={"Nats-Msg-Id": f"deep-dead-letter:{key}:{delivery_count}"},
    )
